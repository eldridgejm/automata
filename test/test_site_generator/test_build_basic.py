...
def test_converts_pages_from_markdown_to_html(demo):
    ...
    # when
    automata._site_generator.build(demo.path, demo.builddir)
...
def test_pages_have_access_to_published_artifacts(demo):
    ...
    # when
    automata._site_generator.build(
...
def test_raises_if_an_unknown_variable_is_accessed_during_page_render(demo):
    ...
    # when
    with raises(automata._site_generator.PageError) as excinfo:
        automata._site_generator.build(demo.path, demo.builddir)
...
def test_accepts_vars(demo):
    ...
    # when
    automata._site_generator.build(demo.path, demo.builddir, vars={"foo": "barbaz"})
