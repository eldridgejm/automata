...
def test_vars_can_be_used_in_markdown_pages(tmpsite, config):
    ...
    # when
    automata.website.generate(config, tmpsite.resources, vars={"foo": "bar"})
...
def test_custom_markdown_engine_can_be_injected(tmpsite, config):
    ...
    automata.website.generate(
        config, tmpsite.resources, render_markdown=custom_markdown_engine
    )
...
def test_generate_handles_all_static_file_types(tmpsite, tmp_path, config):
    ...
    # when
    automata.website.generate(
        config, tmpsite.resources, extra_themes={"default": theme}
    )
...
def test_generate_supports_theme_elements(tmpsite):
    ...
    automata.website.generate(config, tmpsite.resources, extra_themes={"extra": theme})
...
def test_generate_accepts_hooks_parameter(tmpsite, config):
    ...
    # should not raise
    automata.website.generate(config, tmpsite.resources, hooks=hooks)
