...
def test_date_pill_uses_now_for_current_date(tmpsite, config):
    ...
    # when
    automata.website.generate(config, current_time=datetime(2024, 6, 15))
    ...
    # when
    automata.website.generate(config, current_time=datetime(2024, 6, 17))
    output = tmpsite.get_output("index.html")
...
def test_date_pill_with_template_variables(tmpsite, config):
    ...
    automata.website.generate(
        config, current_time=datetime(2024, 6, 15), vars={"foo": "BAR"}
    )
