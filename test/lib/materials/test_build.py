...
def test_build_artifact_integration(example_1):
    # given
    universe = automata.materials.lib.discover(example_1)
    artifact = (
    ...
    # when
    result = automata.materials.lib.build(artifact)
...
def test_build_artifact_when_release_time_is_in_future():
    # given
    artifact = automata.materials.lib.UnbuiltArtifact(
    ...
    # when
    result = automata.materials.lib.build(artifact, run=run, now=now)
...
def test_build_artifact_when_release_time_is_in_future_ignore_release_time():
    ...
    # when
    result = automata.materials.lib.build(
...
def test_build_artifact_when_recipe_is_none():
    ...
    # when
    result = automata.materials.lib.build(artifact, run=run, exists=exists)
...
def test_build_artifact_when_recipe_is_none_raises_if_no_path():
    ...
    # when
    with raises(automata.materials.lib.BuildError):
        result = automata.materials.lib.build(artifact, run=run, exists=exists)
...
def test_build_collection(example_1):
    ...
    # when
    built_universe = automata.materials.lib.build(universe)
