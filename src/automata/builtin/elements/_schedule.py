...
HTML_RESOURCE_SCHEMA: smartconfig.types.Schema = {
    "type": "dict",
    "required_keys": {
        "type": {"type": "string"},
        "html": string_or_template_string(),
    },
    "optional_keys": {
...
MARKDOWN_RESOURCE_SCHEMA: smartconfig.types.Schema = {
    "type": "dict",
    "required_keys": {
        "type": {"type": "string"},
        "markdown": string_or_template_string(),
    },
    "optional_keys": {
        "title": {"type": "string", "nullable": True, "default": None},
        "icon": {"type": "string", "nullable": True, "default": None},
...
METADATA_LINKS_RESOURCE_SCHEMA: smartconfig.types.Schema = {
    "type": "dict",
    "required_keys": {
        "type": {"type": "string"},
        "metadata_key_for_links": {"type": "string"},
        "for_each_link": {
            "type": "dict",
            "required_keys": {
                "text": string_or_template_string(),
                "url": string_or_template_string(),
            },
        },
    },
    "optional_keys": {
        "title": string_or_template_string(nullable=True, default=None),
        "icon": {"type": "string", "nullable": True, "default": None},
        "style": {"type": "string", "nullable": True, "default": "numbered"},
    },
}
...
ACTIVITY_CONFIG_SCHEMA = {
    "type": "dict",
    "required_keys": {
        "title": string_or_template_string(),
        "start_displaying_on": string_or_template_string(),
        "resources": {
            "type": "list",
    ...
    "optional_keys": {
        "week_range_start": string_or_template_string(nullable=True, default=None),
        "week_range_end": string_or_template_string(nullable=True, default=None),
        "due_datetime": string_or_template_string(nullable=True, default=None),
    },
}
...
ACTIVITY_CONFIG_SCHEMA_WITH_DATES = {
    ...
    "optional_keys": {
        "week_range_start": {"type": "date", "nullable": True, "default": None},
        "week_range_end": {"type": "date", "nullable": True, "default": None},
        "due_datetime": {"type": "datetime", "nullable": True, "default": None},
    },
}
...
def make_activity_from_config(
    ...
    """
    config = automata.util.resolution.unwrap_templates(config)
...
def make_activities_from_collection_config(
    ...
    This function is given a collection config of the form:
        ...
        for_each_publication:
          title: !template "Activity for ${ publication.metadata.title }"
          start_displaying_on: !template "${ publication.metadata.release_date }"
          resources:
              ...
        ```
    ...
    # many of the entries of the `for_each_publication` config may be templates
    # whose evaluation is deferred until we have a specific publication to work with.
    # We need to unwrap these templates before passing the config to
    # `resolve_for_each_publication`. However, some of the resource types (e.g.,
    # metadata_links) need to remain as templates until after resolution, because they
    # need to be resolved in a nested loop. To do this, we define a `preserve` function
    # that tells `unwrap_templates` which parts of the config to leave alone.

    def templates_in_metadata_links(config):
        """Do not unwrap templates in metadata_links resource configs (yet)."""
        return (
    ...
    for_each_publication_config = automata.util.resolution.unwrap_templates(
        for_each_publication_config, preserve=templates_in_metadata_links
    )

    # now that templates have been unwrapped as needed, we can resolve the
